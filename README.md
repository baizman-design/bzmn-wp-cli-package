...
+ `update-composer-json-commands.yml`. Updates `composer.json` to automatically add new commands (public methods) added in `src/cli.php` to the `extra > commands` section. (These entries are required by the `wp scaffold package-readme` command.) Requires defining `$REPO_PHP_CLI_SOURCE` and `$REPO_COMMAND_NAME` repository variables.
+ `update-package.yml`. Updates the package on multiple remote hosts. The remote hosts reside in a repository variable, `$REPO_REMOTE_HOSTS`, and correspond to SSH aliases stored in `~/.ssh/config`. This workflow runs manually but could be run automatically.
