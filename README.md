...
+ `sync-composer-json-commands.yml`. Automatically syncs new commands (public methods) added in `src/cli.php` to the `extra > commands` section in `composer.json`. (These entries are required by the `wp scaffold package-readme` command.)
