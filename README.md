...
+ `update-composer-json-commands.yml`. Update `composer.json` to automatically add new commands (public methods) added in `src/cli.php` to the `extra > commands` section. (These entries are required by the `wp scaffold package-readme` command.)
